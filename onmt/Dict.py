...
class Dict(object):
    ...
    def add(self, label, freq=None, idx=None):
    ...
    def prune(self, size, minimum_freq):
        #if size >= self.size():
            #return self

        # Only keep the `size` most frequent entries.
        freq = torch.Tensor(
                [self.frequencies[i] for i in range(2, len(self.frequencies))])
        ...
        for i in idx[:size]:
            i = i.item()
            if minimum_freq != 0 and freq[i] <= minimum_freq:
                break
            newDict.add(self.idxToLabel[i], freq[i])
