...
class Translator(object):
    def __init__(self, opt):
        ...
        self.all_dict = checkpoint['dicts']['all']

        encoder = onmt.Models.Encoder(model_opt, self.all_dict)
        decoder = onmt.Models.Decoder(model_opt, self.all_dict)
        model = onmt.Models.NMTModel(encoder, decoder)

        generator = nn.Sequential(
            nn.Linear(model_opt.rnn_size, self.all_dict.size()),
            nn.LogSoftmax())
